...
window.addEventListener("DOMContentLoaded", function () {
  ...
  Object(_modules_drop__WEBPACK_IMPORTED_MODULE_11__["default"])(); // scrolling('.pageup')
...
var drop = function drop() {
  ...
  fileInput.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      var dots;
      var elemArr = input.files[0].name.split('.'); // 'nameImage.jpg'=>['nameImage', 'jpg']-----так разобьет сплит

      elemArr[0].length > 6 ? dots = '...' : dots = '.';
      var nameElem = elemArr[0].substring(0, 6) + dots + elemArr[1]; //    в предыдущий див записываем имя получившееся (в див выше инпута)
...
var forms = function forms() {
  ...
  inputs = document.querySelectorAll('input'),
      upload = document.querySelectorAll('[name="upload"]'); // для оповещения пользователя об отправке формы
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var dots;
      var elemArr = item.files[0].name.split('.'); // 'nameImage.jpg'=>['nameImage', 'jpg']-----так разобьет сплит
  ...
  form.forEach(function (item) {
    item.addEventListener("submit", function (e) {
      ...
      statusForm.appendChild(textMessage); // собираю все данные которые есть в форме, это и будет тело запроса

      var formDate = new FormData(item);
      var api; // .closest()--------этот метод попробует найти определенный блок по селектору у себя выше по иерархии
      ...
      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question; // отправляем запрос на сервер по адресу 'assets/server.php' с данными которые хранятся в formDate

      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postDate"])(api, formDate) // сейчас вернулся какой-то текст return await res.text()
      .then(function (res) {
        console.log(res);
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    elem.focus(); // .setSelectionRange() ---станавливает начальное и конечное положение выделения текста
  ...
  function createMask() {
    ...
    var val = this.value.replace(/\D/g, ''); // если пользователь начнет удалять +375, то мы ему это делать не разрешим
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  var btn = document.querySelector(trigger); // cards.forEach(el => {
  ...
  btn.addEventListener('click', function () {
    ...
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('http://localhost:3000/styles') // сервер нам возвращает массив и этот массив мы передаем в функцию createCards
    .then(function (res) {
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  var slideIndex = 1;
  var paused = false;
  var items = document.querySelectorAll(slides); // отвечает за перемещение слайд-индекса и самого слайдера соответственно
  ...
  } catch (e) {} // если пользователь навел мышкой на слайд, то он сетинтервал не должен срабатывать(автоплей)
